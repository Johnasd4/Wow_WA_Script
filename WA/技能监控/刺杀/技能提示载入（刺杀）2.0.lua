aura_env.CFG = {
    ...
    SpellList = {
        ...
        {Name = "伏击", KeyHint = "3", ID = 8676, HaveGcd = true},
...
aura_env.UpdateSpecialAura = function()
    ...
    if not aura_env.talent["君王之灾"] then aura_env.spell["君王之灾"].cd = 9999 end
end
...
aura_env.UpdateBurstInfo = function()
    ...
    aura_env.burst.ignore_combat_finish = false
    --死亡猎手版本
    if aura_env.aura["死亡猎手标记"].time > 0 or aura_env.aura["至黑之夜"].time > 0 then
        ...
        if aura_env.aura["徘徊黑暗"].time > 0 and aura_env.aura["徘徊黑暗"].time < 30 and aura_env.spell["毒刃"].cd <= aura_env.aura["徘徊黑暗"].time - 9 then
            aura_env.burst.ignore_combat_finish = true
        ...
        --死印君王
            ...
            if aura_env.talent["君王之灾"] and aura_env.spell["君王之灾"].cd > aura_env.spell["死亡印记"].cd + 7 then
    ...
    --普通版本
        ...
        --死印君王
            ...
            if aura_env.talent["君王之灾"] and aura_env.spell["君王之灾"].cd > aura_env.spell["死亡印记"].cd + 2 then
...
aura_env.SpecialSpellDisplay = function()
    ...
    elseif aura_env.aura["强化锁喉"].time > 0 or aura_env.aura["强化锁喉-潜行"].time > 0 or aura_env.spell["锁喉"].cd > 0 or (not aura_env.talent["连环屠戮"]) then
        if_show = false 
    --显示：强化锁喉buff = 0，战斗中，0 < 君王之灾buff < 12
    elseif aura_env.aura["强化锁喉"].time == 0 and aura_env.in_combat and aura_env.aura["君王之灾"].time > 0 and (aura_env.aura["君王之灾"].time < 6 or (aura_env.aura["君王之灾"].time < 12 and aura_env.talent["诡诈"])) then
        if_show = true                   
    ...
    elseif aura_env.enemy.num_in_range_10 >= 3 and aura_env.aura["锁喉"].effect_unit_num <= aura_env.enemy.num_in_range_10 - 3 and aura_env.in_combat and (not aura_env.burst.mode) then
        if_show = true    
    --显示：10码内敌人 >= 3，（（割裂buff人数 < 10码总人数，割裂buff人数 < 5） | 割裂buff人数 <= 10码总人数 - 3），战斗中，非爆发中
    elseif aura_env.enemy.num_in_range_10 >= 3 and ((aura_env.aura["割裂"].effect_unit_num < 5 and aura_env.aura["割裂"].effect_unit_num < aura_env.enemy.num_in_range_10) or aura_env.aura["割裂"].effect_unit_num <= aura_env.enemy.num_in_range_10 - 3) and aura_env.in_combat and (not aura_env.burst.mode) then
        if_show = true
    --不显示
    else    
        if_show = false
    ...
    if if_show == false then
        ...
        --影遁
        if aura_env.spell["影遁"].cd == 0 and (not IsEncounterInProgress()) and aura_env.talent["死亡猎手标记"] and aura_env.aura["死亡猎手标记"].stack == 0 then
        ...
        --影遁
        elseif aura_env.spell["影遁"].cd == 0 and (not IsEncounterInProgress()) then
...
aura_env.CalculateStageID = function()
    ...
    elseif aura_env.estimate_battle_time_left.battle_time_left <= 3 and aura_env.config.UseCombatEndingStage then
        stage_id = 6        
    --阶段3：战斗时间 <= 爆发时间
    elseif aura_env.estimate_battle_time_left.battle_time_left < aura_env.burst.time and aura_env.config.UseCombatEndingStage and aura_env.burst.ignore_combat_finish == false then
        stage_id = 3
    --阶段4：额外cd <= 爆发准备时间
...
--根据阶段计算应当使用的技能循环
--循环10：非战斗阶段
--循环23：潜行及出潜后6秒，死亡猎手标记天赋，连环屠戮天赋，死亡印记buff > 0
...
aura_env.CalculateLoopID = function()
    ...
    elseif aura_env.stage_id == 2 then
        if aura_env.talent["死亡猎手标记"] then
            --循环23：潜行及出潜后6秒，死亡猎手标记天赋，连环屠戮天赋，死亡印记buff > 0
            if aura_env.talent["连环屠戮"] and aura_env.aura["死亡印记"].time > 0 then 
                loop_id = 23
            --循环22：潜行及出潜后6秒，死亡猎手标记天赋，连环屠戮天赋
            elseif aura_env.talent["连环屠戮"] then 
                loop_id = 22                
            --循环21：潜行及出潜后6秒，死亡猎手标记天赋                
            else
                loop_id = 21
    ...
    elseif aura_env.stage_id == 4 then
        if aura_env.aura["死亡猎手标记"].time > 0 or aura_env.aura["至黑之夜"].time > 0 then
            ...
            if aura_env.talent["君王之灾"] and aura_env.spell["死亡印记"].cd > aura_env.burst.cd and aura_env.spell["君王之灾"].cd == aura_env.burst.cd then
                loop_id = 46
            ...
            elseif aura_env.spell["死亡印记"].cd > aura_env.burst.cd and aura_env.aura["徘徊黑暗"].time > aura_env.spell["毒刃"].cd + 9 and aura_env.spell["毒刃"].cd == aura_env.burst.cd then
                loop_id = 44
            --循环43：（双毒刃君王）爆发前戏，（至黑之夜buff > 0 | 死亡猎手标记buff > 0），死亡印记cd > 爆发cd
            elseif aura_env.talent["君王之灾"] and aura_env.spell["死亡印记"].cd > aura_env.burst.cd then
                loop_id = 43
            -- --循环42：（消失死印君王）爆发前戏，（至黑之夜buff > 0 | 死亡猎手标记buff > 0），徘徊黑暗天赋，非连环屠戮天赋，消失cd <= 爆发cd + 6
...
aura_env.CalculateSpellTypeByLoopID = function(_if_cast_now)
    ...
    if aura_env.loop_id == 10 then
        ...
        --循环切换
            ...
            --循环23：潜行及出潜后6秒，死亡猎手标记天赋，连环屠戮天赋，死亡印记buff > 0
            if aura_env.talent["死亡猎手标记"] and aura_env.talent["连环屠戮"] and aura_env.aura["死亡印记"].time > 0 then 
                aura_env.loop_id = 23           
            --循环22：死亡猎手标记天赋，连环屠戮天赋  
            elseif aura_env.talent["死亡猎手标记"] and aura_env.talent["连环屠戮"] then
                aura_env.loop_id = 22
            --循环21：死亡猎手标记天赋
    ...
    --循环23：潜行及出潜后6秒，死亡猎手标记天赋，连环屠戮天赋，死亡印记buff > 0      
    elseif aura_env.loop_id == 23 then               
        --伏击：潜行buff > 0，死亡猎手标记buff = 0，至黑之夜buff <= 强化锁喉buff
        if aura_env.aura["潜行"].time > 0 and aura_env.aura["死亡猎手标记"].time == 0 and aura_env.aura["至黑之夜"].time <= aura_env.aura["强化锁喉"].time then
            next_spell.name = "伏击" 
        ...
        elseif aura_env.spell["毒刃"].cd == 0 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["毒刃"].time == 0 and aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points >= 6 then
            next_spell.name = "毒刃"             
        --菊花茶：红染之刃天赋，菊花茶层数 >= 2，能量 < 150，菊花茶buff < 1，死亡印记buff > 0    
        elseif aura_env.talent["红染之刃"] and aura_env.spell["菊花茶"].stack >= 2 and aura_env.resource.energy < 150 and aura_env.aura["菊花茶"].time < 1 and aura_env.aura["死亡印记"].time > 0 then
            next_spell.name = "菊花茶"                  
        ...
        elseif aura_env.talent["连环屠戮"] and aura_env.spell["菊花茶"].stack >= 2 and aura_env.aura["菊花茶"].time == 0 and aura_env.spell["君王之灾"].cd == 0 and aura_env.aura["死亡印记"].time > 0 and aura_env.aura["死亡印记"].time < 12 and ((aura_env.aura["毒刃"].time > 0 and aura_env.aura["毒刃"].time < 6) or (aura_env.aura["毒刃"].time == 0 and (not aura_env.talent["轻巧毒刃"]))) then
            next_spell.name = "菊花茶"                       
        --君王之灾：君王之灾cd = 0，0 < 死亡印记buff < 14，（毒刃buff > 0 |（毒刃buff == 0，非轻巧毒刃天赋））    
        elseif aura_env.talent["君王之灾"] and aura_env.spell["君王之灾"].cd == 0 and aura_env.aura["死亡印记"].time > 0 and aura_env.aura["死亡印记"].time < 14 and (aura_env.aura["毒刃"].time > 0 or (aura_env.aura["毒刃"].time == 0 and (not aura_env.talent["轻巧毒刃"]))) then
        ...
        elseif aura_env.aura["至黑之夜"].time > 0 and aura_env.aura["腐蚀飞溅"].time > 0 and aura_env.aura["强化锁喉"].time > 3.3 and aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤"    
        --锁喉：锁喉cd = 0，割裂buff > 0，（锁喉buff = 0 | 非强化锁喉），（诡诈buff > 1.3 | 至黑之夜buff > 3 | 死亡猎手标记buff > 0），连击点 < 7
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["割裂"].time > 0 and (aura_env.aura["锁喉"].time <= 6 or (not aura_env.aura["锁喉"].super)) and (aura_env.aura["诡诈"].time >= 1.3 or aura_env.aura["至黑之夜"].time >= 3 or aura_env.aura["死亡猎手标记"].time > 0) and aura_env.resource.combo_points < 7 then
            next_spell.name = "锁喉"            
        --伏击：（潜行buff > 0 | 诡诈buff > 0），死亡猎手标记buff = 0，至黑之夜buff <= 3，连击点 < 7
        elseif (aura_env.aura["潜行"].time > 0 or aura_env.aura["诡诈"].time > 0) and aura_env.aura["死亡猎手标记"].time == 0 and aura_env.aura["至黑之夜"].time <= 3 and aura_env.resource.combo_points < 7 then
            next_spell.name = "伏击"      
        ...
        elseif aura_env.talent["腐蚀飞溅"] and aura_env.aura["腐蚀飞溅"].time == 0 and aura_env.aura["割裂"].time > 0 and aura_env.aura["夺命药膏"].time > 0 and aura_env.enemy.num_in_range_10 > 1 then
            next_spell.name = "毁伤"               
        --锁喉：锁喉cd = 0，（锁喉buff <= 6 | 非强化锁喉），（连击点 < 5 |（至黑之夜buff > 0，连击点 < 7））
        elseif aura_env.spell["锁喉"].cd == 0 and (aura_env.aura["锁喉"].time <= 6 or (not aura_env.aura["锁喉"].super)) and (aura_env.resource.combo_points < 5 or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points < 7)) then
            next_spell.name = "锁喉"              
        --锁喉：锁喉cd = 0，锁喉buff人数 < 10码总人数，10码总人数 < 10，（连击点 < 5 |（至黑之夜buff > 0，连击点 < 7））
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_unit_num < aura_env.enemy.num_in_range_10 and aura_env.enemy.num_in_range_10 < 10 and (aura_env.resource.combo_points < 5 or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points < 7)) then
            next_spell.name = "锁喉"            
        --锁喉：锁喉cd = 0，锁喉buff人数 < 10码总人数，（2 <= 连击点 < 5 |（至黑之夜buff > 0，4 <= 连击点 < 7））
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_unit_num < aura_env.enemy.num_in_range_10 and ((aura_env.resource.combo_points >= 2 and aura_env.resource.combo_points < 5) or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points >= 4 and aura_env.resource.combo_points < 7)) then
            next_spell.name = "锁喉"                      
        --锁喉：锁喉cd = 0，锁喉buff最小时间 <= 10，0 < 强化锁喉buff < 4，10码总人数 < 10，（连击点 < 5 |（至黑之夜buff > 0，连击点 < 7））
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_min_time <= 10 and aura_env.aura["强化锁喉"].time < 4 and aura_env.enemy.num_in_range_10 < 10 and (aura_env.resource.combo_points < 5 or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points < 7)) then
            next_spell.name = "锁喉"                     
        --锁喉：锁喉cd = 0，锁喉buff最小时间 <= 10，0 < 强化锁喉buff < 4，（2 <= 连击点 < 5 |（至黑之夜buff > 0，4 <= 连击点 < 7））
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_min_time <= 10 and aura_env.aura["强化锁喉"].time < 4 and ((aura_env.resource.combo_points >= 2 and aura_env.resource.combo_points < 5) or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points >= 4 and aura_env.resource.combo_points < 7)) then
            next_spell.name = "锁喉"                 
        --猩红风暴：猩红风暴天赋，猩红风暴buff = 0，割裂buff > 0，连击点 ≥ 5，10码总人数 > 1，猩红风暴buff人数 = 0，（割裂buff人数 >= 5 | 割裂buff人数 = 10码总人数）      
        elseif aura_env.talent["猩红风暴"] and aura_env.aura["猩红风暴"].time == 0 and aura_env.aura["割裂"].time > 0 and aura_env.resource.combo_points >= 5 and aura_env.enemy.num_in_range_10 > 1 and aura_env.aura["猩红风暴"].effect_unit_num == 0 and (aura_env.aura["割裂"].effect_unit_num >= 5 or aura_env.aura["割裂"].effect_unit_num == aura_env.enemy.num_in_range_10) then
            next_spell.name = "猩红风暴"                     
        --割裂：（（割裂buff人数 < 10码总人数，割裂buff人数 < 5） | （割裂buff人数 <= 10码总人数 - 3，10码总人数 < 10） | （割裂buff最小时间 <= 12，10码总人数 < 6，骤死天赋） | （割裂buff = 0，10码总人数 > 1）），（（割裂buff = 0，10码总人数 > 1）| 至黑之夜buff = 0），连击点 >= 5              
        elseif ((aura_env.aura["割裂"].effect_unit_num < 5 and aura_env.aura["割裂"].effect_unit_num < aura_env.enemy.num_in_range_10) or (aura_env.aura["割裂"].effect_unit_num <= aura_env.enemy.num_in_range_10 - 3 and aura_env.enemy.num_in_range_10 < 10) or (aura_env.aura["割裂"].effect_min_time <= 12 and aura_env.enemy.num_in_range_10 < 6 and aura_env.talent["骤死"]) or (aura_env.aura["割裂"].time == 0 and aura_env.enemy.num_in_range_10 > 1)) 
                and ((aura_env.aura["割裂"].time == 0 and aura_env.enemy.num_in_range_10 > 1) or aura_env.aura["至黑之夜"].time == 0) 
                and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                      
        --循环切换
        else
            --阶段5：爆发
    ...
    elseif aura_env.loop_id == 22 then               
        ...
        elseif aura_env.aura["至黑之夜"].time > 0 and aura_env.aura["割裂"].time > 0 and aura_env.aura["腐蚀飞溅"].time == 0 and aura_env.aura["强化锁喉"].time > 4.3 then
            next_spell.name = "毁伤" 
        --君王之灾：君王之灾cd = 0，死亡印记cd > 40，腐蚀飞溅buff = 0
        elseif aura_env.talent["君王之灾"] and aura_env.spell["君王之灾"].cd == 0 and aura_env.spell["死亡印记"].cd > 40 and aura_env.aura["腐蚀飞溅"].time > 0 then
            next_spell.name = "君王之灾" 
        --毒伤：至黑之夜buff > 0, 腐蚀飞溅buff > 0，强化锁喉buff > 3.3，连击点 = 7
        elseif aura_env.aura["至黑之夜"].time > 0 and aura_env.aura["腐蚀飞溅"].time > 0 and aura_env.aura["强化锁喉"].time > 3.3 and aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤"    
    ...
    elseif aura_env.loop_id == 21 then
        ...
        if (aura_env.aura["诡诈"].time > 0 or aura_env.aura["潜行"].time > 0) and aura_env.aura["死亡猎手标记"].time == 0 and aura_env.aura["至黑之夜"].time == 0 then
            next_spell.name = "伏击"         
        --刀扇（起手专用）：割裂buff = 0，见者尽灭buff > 0，消失cd == 0，连击点 < 7
        elseif aura_env.aura["割裂"].time == 0 and aura_env.aura["见者尽灭"].time > 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points < 7 then
            next_spell.name = "刀扇"                                
        --锁喉（起手专用）：锁喉cd = 0，（锁喉buff <= 6，非强化锁喉）割裂buff = 0，消失cd = 0，连击点 < 7，10码总人数 <= 1
        elseif aura_env.spell["锁喉"].cd == 0 and (aura_env.aura["锁喉"].time <= 6 or (not aura_env.aura["锁喉"].super)) and aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points < 7 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "锁喉"   
        --毁伤（起手专用）：割裂buff = 0，消失cd == 0，连击点 < 7，10码总人数 <= 1  
        elseif aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points ~= 7 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "毁伤"                 
        --割裂（起手专用）：割裂buff = 0，消失cd == 0，连击点 = 7，10码总人数 <= 1         
        elseif aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points == 7 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "割裂"                 
    ...
    elseif aura_env.loop_id == 32 then
        ...
        elseif aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤" 
            --等待：能量 <= 65
            if aura_env.resource.energy <= 65 then
    ...
    elseif aura_env.loop_id == 31 then            
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].time == 0 and ((not aura_env.talent["连环屠戮"]) or aura_env.enemy.num_in_range_10 <= 2) and aura_env.resource.combo_points <= 6 then
            next_spell.name = "锁喉"         
        --割裂：割裂buff = 0，连击点 ≥ 5
        elseif aura_env.aura["割裂"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                    
        --猩红风暴：猩红风暴天赋，猩红风暴buff = 0，连击点 ≥ 5，10码总人数 > 1，10码总人数 - 猩红风暴buff人数 > 1      
        elseif aura_env.talent["猩红风暴"] and aura_env.aura["猩红风暴"].time == 0 and aura_env.resource.combo_points >= 5 and aura_env.enemy.num_in_range_10 > 1 and aura_env.enemy.num_in_range_10 - aura_env.aura["猩红风暴"].effect_unit_num >= 1 then
            next_spell.name = "猩红风暴"                                   
        ...
        elseif aura_env.aura["割裂"].time <= 4 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                 
        --割裂：割裂buff <= 补偿时间，连击点 ≥ 5
        elseif aura_env.aura["割裂"].time <= aura_env.resource.combo_points * 1.2 + 1.2 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"     
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_unit_num < aura_env.enemy.num_in_range_10 and aura_env.enemy.num_in_range_10 <= 2 and (aura_env.resource.combo_points == 0 or aura_env.resource.combo_points == 6) then
            next_spell.name = "锁喉"  
            next_spell.type = 3                        
        --割裂（技能类型3）：割裂buff人数 < 10码内敌人，10码内敌人 <= 3，割裂buff最小时间 <= 补偿时间，（毒伤buff >= 4 | 毒伤buff = 0），连击点 ≥ 5
        elseif aura_env.aura["割裂"].effect_unit_num < aura_env.enemy.num_in_range_10 and aura_env.enemy.num_in_range_10 <= 3 and aura_env.aura["割裂"].effect_min_time <= aura_env.resource.combo_points * 1.2 + 1.2 and (aura_env.aura["毒伤"].time >= 4 or aura_env.aura["毒伤"].time == 0) and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"  
            next_spell.type = 3        
        --毁伤：腐蚀飞溅天赋，腐蚀飞溅buff = 0，割裂buff > 0，夺命药膏buff > 0，10码总人数 > 1
        elseif aura_env.talent["腐蚀飞溅"] and aura_env.aura["腐蚀飞溅"].time == 0 and aura_env.aura["割裂"].time > 0 and aura_env.aura["夺命药膏"].time > 0 and aura_env.enemy.num_in_range_10 > 1 then
        ...
        elseif aura_env.resource.combo_points >= 5 then
            ...
            elseif aura_env.aura["死亡猎手标记"].stack == 3 and aura_env.enemy.num_in_range_10 <= 1 then
                next_spell.if_cast_now = false                          
            --等待：（毒伤buff >= 补偿时间 | 毒伤buff = 0），割裂buff > 8，增效药膏层数 <= 18
            elseif (aura_env.aura["毒伤"].time >= aura_env.resource.combo_points * 0.3 or aura_env.aura["毒伤"].time == 0) and aura_env.aura["割裂"].time > 8 and aura_env.aura["增效药膏"].stack <= 18 then
                next_spell.if_cast_now = false
            end        
        --毁伤：侧袭buff > 0
        elseif aura_env.aura["侧袭"].time > 0 then
            ...
            if aura_env.enemy.num_in_range_10 > 2 then
                next_spell.if_cast_now = true                
            --等待：能量 < 80
            elseif aura_env.resource.energy < 80 then
                next_spell.if_cast_now = false
            end        
        end             
    --循环30：平稳    
    ...
    elseif aura_env.loop_id == 46 then    
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].time == 0 and ((not aura_env.talent["连环屠戮"]) or aura_env.enemy.num_in_range_10 <= 2) and aura_env.resource.combo_points <= 6 then
            next_spell.name = "锁喉"         
        --割裂：割裂buff = 0，至黑之夜buff = 0，连击点 ≥ 5   
        elseif aura_env.aura["割裂"].time == 0 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                    
        --猩红风暴：猩红风暴天赋，猩红风暴buff = 0，至黑之夜buff = 0，连击点 ≥ 5，10码总人数 > 1，10码总人数 - 猩红风暴buff人数 > 1      
        elseif aura_env.talent["猩红风暴"] and aura_env.aura["猩红风暴"].time == 0 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 and aura_env.enemy.num_in_range_10 > 1 and aura_env.enemy.num_in_range_10 - aura_env.aura["猩红风暴"].effect_unit_num >= 1 then
            next_spell.name = "猩红风暴"                                
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].time <= 6 and (not aura_env.aura["锁喉"].super) and ((not aura_env.talent["连环屠戮"]) or aura_env.enemy.num_in_range_10 <= 2) and aura_env.resource.combo_points <= 6 then
            next_spell.name = "锁喉"       
        --割裂：割裂buff <= 4，至黑之夜buff = 0，连击点 ≥ 5      
        elseif aura_env.aura["割裂"].time <= 4 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                 
        --割裂：割裂buff <= 补偿时间，连击点 ≥ 5，死亡猎手标记buff层数 >= 2
        elseif aura_env.aura["割裂"].time <= aura_env.resource.combo_points * 1.2 + 1.2 and aura_env.resource.combo_points >= 5 and aura_env.aura["死亡猎手标记"].stack >= 2 then
            next_spell.name = "割裂"     
        --猩红风暴：猩红风暴天赋，猩红风暴buff <= 5，至黑之夜buff = 0，连击点 ≥ 5，10码总人数 > 1         
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].effect_unit_num < aura_env.enemy.num_in_range_10 and aura_env.enemy.num_in_range_10 <= 2 and aura_env.resource.combo_points <= 6 then
            next_spell.name = "锁喉"  
            next_spell.type = 3                        
        --割裂（技能类型3）：割裂buff人数 < 10码内敌人，10码内敌人 <= 3，割裂buff最小时间 <= 补偿时间，（毒伤buff >= 4 | 毒伤buff = 0），至黑之夜buff = 0，连击点 ≥ 5
        elseif aura_env.aura["割裂"].effect_unit_num < aura_env.enemy.num_in_range_10 and aura_env.enemy.num_in_range_10 <= 3 and aura_env.aura["割裂"].effect_min_time <= aura_env.resource.combo_points * 1.2 + 1.2 and (aura_env.aura["毒伤"].time >= 4 or aura_env.aura["毒伤"].time == 0) and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"  
            next_spell.type = 3    
        --毁伤：腐蚀飞溅天赋，腐蚀飞溅buff = 0，割裂buff > 0，夺命药膏buff > 0，10码总人数 > 1
        elseif aura_env.talent["腐蚀飞溅"] and aura_env.aura["腐蚀飞溅"].time == 0 and aura_env.aura["割裂"].time > 0 and aura_env.aura["夺命药膏"].time > 0 and aura_env.enemy.num_in_range_10 > 1 then
        ...
        elseif aura_env.aura["见者尽灭"].time > 0 and aura_env.resource.combo_points < 5 then
            next_spell.name = "刀扇"                                  
        --毒伤：(至黑之夜buff = 0，连击点 >= 5) | 连击点 = 7
        elseif (aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5) or aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤"     
            --不等待：至黑之夜buff > 0
            if aura_env.aura["至黑之夜"].time > 0 then
                next_spell.if_cast_now = true             
            --不等待：能量 > 250
            elseif aura_env.resource.energy > 250 then
                next_spell.if_cast_now = true       
            --不等待：10码总人数 > 2
            elseif aura_env.enemy.num_in_range_10 > 2 then
                next_spell.if_cast_now = true                
    ...
    elseif aura_env.loop_id == 45 then    
        --爆发：爆发cd = 0，割裂buff > 12
        if aura_env.burst.cd == 0 and aura_env.aura["割裂"].time > 12 then
            if aura_env.aura["徘徊黑暗"].time > 0 then
                next_spell.if_cast_now = true
        ...
        elseif aura_env.aura["见者尽灭"].time > 0 and aura_env.resource.combo_points < 5 then
            next_spell.name = "刀扇"          
        --割裂：爆发cd < 5，至黑之夜buff = 0，割裂buff <= 爆发cd + 12，连击点 ≥ 5
        elseif aura_env.burst.cd < 5 and aura_env.aura["至黑之夜"].time == 0 and aura_env.aura["割裂"].time <= aura_env.burst.cd + 12 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                               
        --毒伤：(至黑之夜buff = 0，连击点 >= 5) | 连击点 = 7
        elseif (aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5) or aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤"     
            ...
            if aura_env.aura["至黑之夜"].time > 0 then
                next_spell.if_cast_now = true             
            --不等待：能量 > 200
            elseif aura_env.resource.energy > 200 then
                next_spell.if_cast_now = true       
            --不等待：10码总人数 > 2
            elseif aura_env.enemy.num_in_range_10 > 2 then
                next_spell.if_cast_now = true                             
    ...
    elseif aura_env.loop_id == 44 then  
        --爆发：爆发cd = 0，割裂buff > 12，（死亡猎手标记buff层数 = 1 | （至黑之夜buff > 0，连击点 >= 6））
        if aura_env.burst.cd == 0 and aura_env.aura["割裂"].time > 12 and (aura_env.aura["死亡猎手标记"].stack == 1 or (aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points >= 6)) then
    ...
    elseif aura_env.loop_id == 41 then
        ...
        if aura_env.burst.cd == 0 and aura_env.aura["割裂"].time > 12 and
            ...
            aura_env.loop_id = 51
            return aura_env.CalculateSpellTypeByLoopID(next_spell.if_cast_now)   
        --单体起手专用：割裂buff = 0，（10码总人数 <= 1 | 非连环屠戮天赋）
        elseif aura_env.aura["切割"].time == 0 and aura_env.aura["割裂"].time == 0 and (aura_env.enemy.num_in_range_10 <= 1 or (not aura_env.talent["连环屠戮"])) then     
            --刀扇（起手专用）：割裂buff = 0，见者尽灭buff > 0，消失cd == 0，连击点 < 7
            if aura_env.aura["割裂"].time == 0 and aura_env.aura["见者尽灭"].time > 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points < 7 then
                next_spell.name = "刀扇"                         
            --锁喉（起手专用）：锁喉cd = 0，（锁喉buff <= 6，非强化锁喉），割裂buff = 0，消失cd = 0，连击点 < 7
            elseif aura_env.spell["锁喉"].cd == 0 and (aura_env.aura["锁喉"].time <= 6 or (not aura_env.aura["锁喉"].super)) and aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points < 7 then
                next_spell.name = "锁喉"          
            --毁伤（起手专用）：割裂buff = 0，消失cd == 0，连击点 < 7
            elseif aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points < 7 then
                next_spell.name = "毁伤"                 
            --割裂（起手专用）：割裂buff = 0，消失cd == 0，连击点 = 7
            elseif aura_env.aura["割裂"].time == 0 and aura_env.spell["消失"].cd == 0 and aura_env.resource.combo_points == 7 then
                next_spell.name = "割裂"              
            end
        --割裂：0 < 割裂buff < 1，连击点 > 0         
    ...
    elseif aura_env.loop_id == 40 then
        --爆发：爆发cd = 0，割裂buff > 爆发时间 + 3，（锁喉buff > 9 | 强化锁喉），（非猩红风暴天赋 | 猩红风暴buff >= 5 | 10码总人数 <= 1）
        if aura_env.burst.cd == 0 and
                aura_env.aura["割裂"].time > aura_env.burst.time + 3 and (aura_env.aura["锁喉"].time > 9 or aura_env.aura["锁喉"].super or (aura_env.talent["连环屠戮"] and aura_env.enemy.num_in_range_10 > 2)) and
        ...
        elseif aura_env.burst.cd < 2 and aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].time <= aura_env.burst.cd + 9 and (not aura_env.aura["锁喉"].super) and ((not aura_env.talent["连环屠戮"]) or aura_env.enemy.num_in_range_10 <= 2) then
            next_spell.name = "锁喉"                                
        --割裂：爆发cd < 5，割裂buff <= 爆发cd + 爆发时间 + 3，连击点 ≥ 5
        elseif aura_env.burst.cd < 5 and aura_env.aura["割裂"].time <= aura_env.burst.cd + aura_env.burst.time + 3 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                               
        --毒伤：(至黑之夜buff = 0，连击点 >= 5) | 连击点 = 7
        elseif (aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5) or aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤"           
            --不等待：能量 > 250
            if aura_env.resource.energy > 250 then
                next_spell.if_cast_now = true       
            --不等待：10码总人数 > 2
            elseif aura_env.enemy.num_in_range_10 > 2 then
                next_spell.if_cast_now = true                
    ...
    elseif aura_env.loop_id == 53 then
        --毒刃（君王起手毒刃）：轻巧毒刃天赋，毒刃层数 >= 1，毒刃充能cd <= 15，连击点 >= 6，君王之灾cd = 0
        if aura_env.talent["轻巧毒刃"] and aura_env.spell["毒刃"].stack >= 1 and aura_env.spell["毒刃"].charge_cd <= 15 and aura_env.spell["君王之灾"].cd == 0 then
            next_spell.name = "毒刃"       
        ...
        elseif aura_env.spell["锁喉"].cd == 0 and aura_env.aura["锁喉"].time <= 9 and (not aura_env.aura["锁喉"].super) and ((not aura_env.talent["连环屠戮"]) or aura_env.enemy.num_in_range_10 <= 2) and ((aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points == 6) or (aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points == 4)) then
            next_spell.name = "锁喉"                       
        --割裂：割裂buff <= 1，至黑之夜buff = 0，连击点 > 0         
        elseif aura_env.aura["割裂"].time <= 1 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points > 0 then
            next_spell.name = "割裂"    
        --割裂：割裂buff <= 君王之灾buff（不存在算14） + 3，毒刃buff = 0，至黑之夜buff = 0，连击点 >= 5         
        elseif (aura_env.aura["割裂"].time <= aura_env.aura["君王之灾"].time + 3 or (aura_env.aura["君王之灾"].time == 0 and aura_env.aura["割裂"].time <= 17)) and aura_env.aura["毒刃"].time == 0 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                  
        --猩红风暴：猩红风暴天赋，猩红风暴buff <= 5，至黑之夜buff = 0，连击点 >= 5，10码总人数 > 1     
        elseif aura_env.talent["猩红风暴"] and aura_env.aura["猩红风暴"].time <= 5 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 and aura_env.enemy.num_in_range_10 > 1 then
            next_spell.name = "猩红风暴"                    
        ...
        elseif aura_env.aura["见者尽灭"].time > 0 and aura_env.resource.combo_points < 5 then
            next_spell.name = "刀扇"               
        --剧毒之刃：至黑之夜buff > 0，连击点 = 6，8 < 增效药膏buff层数 < 10，10码总人数 <= 1
        elseif aura_env.aura["至黑之夜"].time > 0 and aura_env.resource.combo_points == 6 and aura_env.aura["增效药膏"].stack > 8 and aura_env.aura["增效药膏"].stack < 10 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "剧毒之刃"                  
        --毒伤：（连击点 >= 5，至黑之夜buff = 0） | 连击点 = 7
        elseif (aura_env.resource.combo_points >= 5 and aura_env.aura["至黑之夜"].time == 0) or aura_env.resource.combo_points == 7 then
            next_spell.name = "毒伤" 
        --技能类型1
        else
            next_spell.type = 1
    ...
    elseif aura_env.loop_id == 51 then
        ...
        --消失：非连环屠戮天赋，刺客大师天赋，消失cd = 0，0 < 君王之灾buff < 6
        elseif (not aura_env.talent["连环屠戮"]) and aura_env.talent["刺客大师"] and aura_env.spell["消失"].cd == 0 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time < 6 then
            next_spell.name = "消失"                 
        --毒刃：轻巧毒刃天赋，毒刃cd = 0，毒刃buff = 0，君王之灾buff = 0，（连击点 >= 6 | （连击点 >= 5，死亡印记buff < 13））
        elseif aura_env.talent["轻巧毒刃"] and aura_env.spell["毒刃"].cd == 0 and aura_env.aura["毒刃"].time == 0 and aura_env.aura["君王之灾"].time == 0 and (aura_env.resource.combo_points >= 6 or (aura_env.resource.combo_points >= 5 and aura_env.aura["死亡印记"].time < 13)) then
            next_spell.name = "毒刃"                                 
        --毒刃：轻巧毒刃天赋，毒刃cd = 0，毒刃buff < 1，0 < 君王之灾buff < 8，连击点 < 7
        elseif aura_env.talent["轻巧毒刃"] and aura_env.spell["毒刃"].cd == 0 and aura_env.aura["毒刃"].time < 1 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time < 8 and aura_env.aura["死亡猎手标记"].stack <= 1 and aura_env.resource.combo_points < 7 then
            next_spell.name = "毒刃"      
        --毒刃：非轻巧毒刃天赋，毒刃cd = 0，毒刃buff < 1，0 < 君王之灾buff < 8，连击点 < 7
        elseif (not aura_env.talent["轻巧毒刃"]) and aura_env.spell["毒刃"].cd == 0 and aura_env.aura["毒刃"].time < 1 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time < 8 and aura_env.resource.combo_points < 7 then
            next_spell.name = "毒刃"       
        --毒刃（君王后置毒刃）：毒刃cd = 0，君王之灾buff > 0，毒刃buff = 0，至黑之夜buff > 0，（连击点 >= 6 | （连击点 >= 5，至黑之夜buff = 0））
        elseif aura_env.spell["毒刃"].cd == 0 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["毒刃"].time == 0 and aura_env.aura["至黑之夜"].time > 0 and (aura_env.resource.combo_points >= 6 or (aura_env.resource.combo_points >= 5 and aura_env.aura["至黑之夜"].time == 0)) then
            next_spell.name = "毒刃"            
        --毒刃（君王后置毒刃）：非君王之灾天赋，毒刃cd = 0，毒刃buff = 0，，（连击点 >= 6 | （连击点 >= 5，至黑之夜buff = 0））
        elseif (not aura_env.talent["君王之灾"]) and aura_env.spell["毒刃"].cd == 0 and aura_env.aura["毒刃"].time == 0 and aura_env.aura["死亡猎手标记"].stack <= 1 and (aura_env.resource.combo_points >= 6 or (aura_env.resource.combo_points >= 5 and aura_env.aura["至黑之夜"].time == 0)) then
            next_spell.name = "毒刃"                
        --菊花茶：红染之刃天赋，菊花茶层数 >= 2，能量 < 150，菊花茶buff < 1，死亡印记buff > 0    
        elseif aura_env.talent["红染之刃"] and aura_env.spell["菊花茶"].stack >= 2 and aura_env.resource.energy < 150 and aura_env.aura["菊花茶"].time < 1 and aura_env.aura["死亡印记"].time > 0 then
            next_spell.name = "菊花茶"                  
        ...
        elseif aura_env.talent["连环屠戮"] and aura_env.spell["菊花茶"].stack >= 2 and aura_env.aura["菊花茶"].time == 0 and aura_env.spell["君王之灾"].cd == 0 and aura_env.aura["死亡印记"].time > 0 and aura_env.aura["死亡印记"].time < 12 and ((aura_env.aura["毒刃"].time > 0 and aura_env.aura["毒刃"].time < 6) or (aura_env.aura["毒刃"].time == 0 and (not aura_env.talent["轻巧毒刃"]))) then
            next_spell.name = "菊花茶"                       
        --君王之灾：君王之灾cd = 0，0 < 死亡印记buff < 14，（毒刃buff > 0 |（毒刃buff == 0，非轻巧毒刃天赋））    
        elseif aura_env.spell["君王之灾"].cd == 0 and aura_env.aura["死亡印记"].time > 0 and aura_env.aura["死亡印记"].time < 14 and (aura_env.aura["毒刃"].time > 0 or (aura_env.aura["毒刃"].time == 0 and (not aura_env.talent["轻巧毒刃"]))) then
            next_spell.name = "君王之灾"    
        --菊花茶：菊花茶层数 > 0，菊花茶buff = 0，0 < 君王之灾buff <= 5     
        elseif aura_env.spell["菊花茶"].stack > 0 and aura_env.aura["菊花茶"].time == 0 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time <= 5 then
            next_spell.name = "菊花茶"
        ...
        elseif aura_env.aura["割裂"].time < 1 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points > 0 then
            next_spell.name = "割裂"    
        --割裂：割裂buff <= 10，毒刃buff = 0，至黑之夜buff = 0，连击点 >= 5         
        elseif aura_env.aura["割裂"].time <= 12 and aura_env.aura["毒刃"].time == 0 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 then
            next_spell.name = "割裂"                  
        --猩红风暴：猩红风暴天赋，猩红风暴buff <= 5，至黑之夜buff = 0，连击点 >= 5，10码总人数 > 1     
        elseif aura_env.talent["猩红风暴"] and aura_env.aura["猩红风暴"].time <= 5 and aura_env.aura["至黑之夜"].time == 0 and aura_env.aura["至黑之夜"].time == 0 and aura_env.resource.combo_points >= 5 and aura_env.enemy.num_in_range_10 > 1 then
            next_spell.name = "猩红风暴"                    
        --毁伤：腐蚀飞溅天赋，腐蚀飞溅buff = 0，割裂buff > 0，夺命药膏buff > 0，10码总人数 > 1
        elseif aura_env.talent["腐蚀飞溅"] and aura_env.aura["腐蚀飞溅"].time == 0 and aura_env.aura["割裂"].time > 0 and aura_env.aura["夺命药膏"].time > 0 and aura_env.enemy.num_in_range_10 > 1 then
            next_spell.name = "毁伤"   
    ...
    elseif aura_env.loop_id == 50 then
        ...
        elseif aura_env.spell["毒刃"].cd == 0 and aura_env.aura["毒刃"].time < 1 and aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time < 8 then
            next_spell.name = "毒刃"       
        --菊花茶：（（菊花茶层数 >= 2，能量 < 150）| 菊花茶层数 = 3），菊花茶buff < 1，死亡印记buff > 0，君王之灾buff = 0        
        elseif ((aura_env.spell["菊花茶"].stack >= 2 and aura_env.resource.energy < 150) or aura_env.spell["菊花茶"].stack == 3) and aura_env.aura["菊花茶"].time < 1 and aura_env.aura["死亡印记"].time > 0 and aura_env.aura["君王之灾"].time == 0 then
            next_spell.name = "菊花茶"            
        --君王之灾：君王之灾cd = 0          
...
aura_env.CalculateSpellBySpellType = function()
    ...
    elseif aura_env.next_spell.type == 1 then
        --毁伤：10码目标 <= 2，死亡猎手标记-自身buff > 0
        if aura_env.enemy.num_in_range_10 <= 2 and aura_env.aura["死亡猎手标记-自身"].time > 0 then
        --刀扇：10码目标 >= 2
        elseif aura_env.enemy.num_in_range_10 >= 2 then
    ...
    --诡诈状态下，有强化伏击天赋，毁伤替换为伏击
    if aura_env.aura["诡诈"].time > 0 and aura_env.next_spell.name == "毁伤" and aura_env.talent["强化伏击"] then
        aura_env.next_spell.name = "伏击"
    end
    --等待：能量 < 80
    if aura_env.resource.energy < 80 then
...
aura_env.UpdateResouceByCastSpell = function(_index)
    ...
    elseif aura_env.next_spell.name == "伏击" then
        ...
        aura_env.aura["死亡猎手标记-自身"].time = 0
        aura_env.aura["增效药膏"].time = 12
        aura_env.aura["夺命药膏"].time = 12
    ...
    elseif aura_env.next_spell.name == "毒伤" then
        ...
        if aura_env.aura["至黑之夜"].time > 0 and final_skill_combo_points == 7 then
            if aura_env.aura["死亡猎手标记"].time == 0 then
                ...
                aura_env.aura["死亡猎手标记-自身"].time = 12
