function(allstates,event,...)
    ...
    if event == "FRAME_UPDATE" then
        ...
        local showGMGJ = false       --鬼魅攻击图标是否显示
        ...
        if loopID == 1 then             
            ...
            if aura_env.spell.GMGJ.cd == 0 then
                if aura_env.aura.QSLJ.time > 0 then
                    aura_env.burstComboPoints = math.min(aura_env.comboPointsMax, aura_env.burstComboPoints + 2)
                else 
                    aura_env.burstComboPoints = math.min(aura_env.comboPointsMax, aura_env.burstComboPoints + 1)
                end
            end
            ...
            --非潜行，剑刃乱舞buff=0，冲动cd=0，连击点>0或切割buff>4(常数)
            --非潜行，剑刃乱舞buff=0，爆发连击点＞4，切割buff>4(常数)
            --潜行，剑刃乱舞buff=0，冲动buff＞3，爆发连击点＞4，切割buff>4(常数)
            elseif (aura_env.spell.JRLW.cd == 0 and (not (aura_env.aura.QX.aura or aura_env.aura.XS.aura)) and aura_env.aura.JRLW.time == 0 and aura_env.aura.CD.time == 0 and (aura_env.comboPoints > 0 or aura_env.aura.QG.time > aura_env.loop1supplyTimeQG))
                or (aura_env.spell.JRLW.cd == 0 and (not (aura_env.aura.QX.aura or aura_env.aura.XS.aura)) and aura_env.aura.JRLW.time == 0 and aura_env.burstComboPoints > 4 and aura_env.aura.QG.time > aura_env.loop1supplyTimeQG)
                or (aura_env.spell.JRLW.cd == 0 and (aura_env.aura.QX.aura or aura_env.aura.XS.aura) and aura_env.aura.JRLW.time == 0 and aura_env.aura.CD.time > 3 and aura_env.burstComboPoints > 4 and aura_env.aura.QG.time > aura_env.loop1supplyTimeQG) then
                spellTypeID = 3                     --技能直接释放
            ...
            elseif aura_env.spell.MYGT.cd == 0 and (aura_env.diceMaxTime <= aura_env.loop1supplyTimeBeforeBurstMYGT or aura_env.diceReroll == true) then      
                ...
                keyHint = aura_env.config.keyMYGT   --快捷键       
            --剑刃乱舞buff=0，目标＞1
            elseif aura_env.spell.JRLW.cd == 0 and aura_env.aura.JRLW.time == 0 and aura_env.enemyCount > 1 then
                spellTypeID = 3                     --技能直接释放
                spell = aura_env.spell.JRLW         --剑刃乱舞               
                highlightType = 3                   --高亮发光
                keyHint = aura_env.config.keyJRLW   --快捷键                  
        ...
        if loopID == 2 then
            ...
            --鬼魅攻击cd=0
            if aura_env.spell.GMGJ.cd == 0 and aura_env.aura.CD.time > 0 then
                showGMGJ = true
                if aura_env.aura.QSLJ.time > 0 then
                    aura_env.comboPoints = math.min(aura_env.comboPointsMax, aura_env.comboPoints + 2)
                else 
                    aura_env.comboPoints = math.min(aura_env.comboPointsMax, aura_env.comboPoints + 1)
                end
            end
        ...
        if loopID == 3 then
            ...
            or (aura_env.aura.JRLW.time == 0)
            or (aura_env.aura.CD.time == 0 and aura_env.aura.MCDBZ.time == 0) then
            ...
            --非即将爆发，鬼魅攻击cd=0
            elseif aura_env.spell.GMGJ.cd == 0 and aura_env.aura.CD.time > 0 and aura_env.goingBurst == false then
                showGMGJ = true
                if aura_env.aura.QSLJ.time > 0 then
                    aura_env.comboPoints = math.min(aura_env.comboPointsMax, aura_env.comboPoints + 2)
                else 
            ...
            elseif aura_env.aura.QG.time <= aura_env.loop3supplyTimeQG and aura_env.aura.JRLW.time == 0 and aura_env.comboPoints > 4 then      
                ...
                if aura_env.goingBurst == true then                     
                    --骰子buff≤7(常数)，爆发连击点>5
                    if aura_env.spell.MYGT.cd == 0 and aura_env.diceMaxTime <= aura_env.loop3supplyTimeBeforeBurstMYGT and aura_env.burstComboPoints > 5 then
                        spellTypeID = 3                     --技能直接释放
                        spell = aura_env.spell.MYGT         --命运骨骰               
                        highlightType = 3                   --高亮发光
                        keyHint = aura_env.config.keyMYGT   --快捷键
        ...
        if loopID == 4 then
            ...
            --鬼魅攻击cd=0，冲动buff＞0，正中眉心cd > 0
            if aura_env.spell.GMGJ.cd == 0 and aura_env.aura.CD.time > 0 and aura_env.spell.ZZMX.cd > 0 then
                showGMGJ = true
                if aura_env.aura.QSLJ.time > 0 then
                    aura_env.comboPoints = math.min(aura_env.comboPointsMax, aura_env.comboPoints + 2)
                else 
            ...
            --冲动buff=0，爆发时间<6
            --爆发连击点≤2，冲动buff≤5(常数)
            --连击点≤2，正中眉心cd=0，冲动buff≤5(常数)
            --连击点≤2，正中眉心cd=0，暗影之舞cd=0或消失cd=0
            ...
            or (aura_env.spell.CD.cd == 0 and aura_env.comboPoints <= 2 and aura_env.spell.ZZMX.cd == 0 and aura_env.aura.CD.time <= aura_env.loop4supplyTimeCD) 
            or (aura_env.spell.CD.cd == 0 and aura_env.comboPoints <= 2 and aura_env.spell.ZZMX.cd == 0 and (aura_env.spell.XS.cd == 0 or aura_env.spell.AYZW.cd == 0)) then
                --冲动，连击点视为7，冲动buff=20，灌铅骰子buff=45，骰子重新判定
                showCD = true
                aura_env.comboPoints = aura_env.comboPointsMax
                aura_env.burstComboPoints = aura_env.comboPointsMax
        ...
        if spellTypeID == 1 then
            local factorQSLJ = 0
            if aura_env.aura.QSLJ.time > 0 then factorQSLJ = 1 end
            local expectComboPointsJRLW = aura_env.enemyCountJRLW + factorQSLJ
            local expectComboPointsSCHX = 2 + factorQSLJ
            local minComboPointsYX = 1 + factorQSLJ
            local maxComboPointsYX = (1 + factorQSLJ) * 2

            --爆发期
            if aura_env.burstTime > 0 then
                --爆发连击点+剑刃乱舞期望连击点＞4，目标＞2
                if aura_env.spell.JRLW.cd == 0 and aura_env.burstComboPoints + expectComboPointsJRLW > 4 and aura_env.enemyCountJRLW > 2 then
                    spell = aura_env.spell.JRLW         --剑刃乱舞               
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keyJRLW   --快捷键     
                --爆发连击点+申斥回响期望连击点＞2    
                elseif aura_env.spell.SCHX.cd == 0 and aura_env.burstComboPoints + expectComboPointsSCHX > 4 then        
                    spell = aura_env.spell.SCHX         --申斥回响               
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keySCHX   --快捷键                                            
                --乘胜追击层数=2
                --乘胜追击层数>0，爆发连击点+影袭最低连击点≤4，正中眉心cd=0
                elseif aura_env.aura.KCZJ.stack == 2
                or (aura_env.aura.KCZJ.stack > 0 and aura_env.burstComboPoints + minComboPointsYX <= 4 and aura_env.spell.ZZMX.cd == 0) then
                    spell = aura_env.spell.SQSJ         --手枪射击
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keySQSJ   --快捷键                        
                --爆发连击点+影袭最高连击点≤4，目标＞2
                elseif aura_env.spell.JRLW.cd == 0 and aura_env.burstComboPoints + maxComboPointsYX <= 4 and aura_env.enemyCountJRLW > 2 then
                    spell = aura_env.spell.JRLW         --剑刃乱舞               
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keyJRLW   --快捷键                     
                --默认
                else
                    spell = aura_env.spell.YX           --影袭
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keyYXFJ   --快捷键        
                end
            --平稳期
            else
                --目标＞2，连击点≤2或连击点+剑刃乱舞期望连击点≤7
                if aura_env.spell.JRLW.cd == 0 and aura_env.enemyCountJRLW > 2 and (aura_env.comboPoints <= 2 or aura_env.comboPoints + expectComboPointsJRLW <= 7) then
                    spell = aura_env.spell.JRLW         --剑刃乱舞               
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keyJRLW   --快捷键      
                --连击点+申斥回响期望连击点<=7
                elseif aura_env.spell.SCHX.cd == 0 and aura_env.comboPoints + expectComboPointsSCHX <= 7 then        
                    spell = aura_env.spell.SCHX         --申斥回响               
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keySCHX   --快捷键                                          
                --乘胜追击层数=2，非即将爆发
                --乘胜追击层数>0，连击点≤3(强势连击buff>0则为2)，目标<=2，非即将爆发
                elseif (aura_env.aura.KCZJ.stack == 2 and aura_env.goingBurst == false) 
                    ...
                    or (aura_env.aura.KCZJ.stack > 0 and aura_env.comboPoints <= 2 and aura_env.aura.QSLJ.time > 0 and aura_env.enemyCount <= 2 and aura_env.goingBurst == false) then
                    spell = aura_env.spell.SQSJ         --手枪射击
                    highlightType = 3                   --高亮发光
                    keyHint = aura_env.config.keySQSJ   --快捷键   
                --默认               
        ...
        if aura_env.spell.GCD.cd > aura_env.gcd - aura_env.config.iconSwitchDelay then
            ...
            showGMGJ = aura_env.preStatus.showGMGJ
        else
            --记录上一轮信息
            aura_env.preStatus.showCD = showCD
            aura_env.preStatus.showXS = showXS
            aura_env.preStatus.showAYZW = showAYZW
            aura_env.preStatus.showGMGJ = showGMGJ
        end

        --设置额外图标可见
        aura_env.updateExtraIconVisible("狂徒—技能提示：时运—冲动", showCD)          
        aura_env.updateExtraIconVisible("狂徒—技能提示：时运—消失", showXS)           
        aura_env.updateExtraIconVisible("狂徒—技能提示：时运—暗影之舞", showAYZW)   
        --不在特殊技能补充的时候打鬼魅攻击
        if spellTypeID == 3 then
            showGMGJ = false
        end 
        aura_env.updateExtraIconVisible("狂徒—技能提示：时运—鬼魅攻击", showGMGJ)    
