aura_env.CFG = {
    ...
    SpellList = {
        ...
        {Name = "消失大枪", KeyHint = "C1", ID = 1856, HaveGcd = true},
        {Name = "鬼魅攻击", KeyHint = "C2", ID = 196937, HaveGcd = false},
        ...
        {Name = "冲动", KeyHint = "CF", ID = 13750, HaveGcd = false}
    },
    ...
    AuraList = {
        ...
        {Name = "大乱斗", ID = 139958, IfSelfAura = true, IfHelpfulAura = true, IfCheckEffectUnitNum = false, TimeMax = 9999},
...
aura_env.Init = function()
    ...
    for dice in aura_env.CFG.DiceName do
...
aura_env.OnSpellCast = function(_spell_name)
    ...
    if _spell_name == "命运骨骰" then
        aura_env.status.dice_lock = false
    end
end
...
aura_env.UpdateSpecialAura = function() 
    --骰子全局状态更新
    for dice in aura_env.CFG.DiceName do
        if aura_env.spell[dice].time > 0 then       
            if aura_env.aura[dice].time > aura_env.status.dice_time[dice] then
                aura_env.aura[dice].lock = true
            else
...
aura_env.SpecialSpellDisplay = function()
    ...
    elseif aura_env.spell["消失"].cd == 0 and aura_env.enemy.num_in_range_8 >= 3 and aura_env.aura["锁喉"].effect_unit_num <= aura_env.enemy.num_in_range_8 - 3 and aura_env.in_combat and (not aura_env.burst.mode) then
        region:Show()
    --不显示
    else    
        region:Hide()
...
aura_env.CalculateSpellTypeByLoopID = function(_if_cast_now)
    ...
    elseif aura_env.loop_id == 22 then
        --割裂：割裂buff = 0，死亡印记cd <= 10，连击点 > 0，10码总人数 <= 1         
        if aura_env.aura["割裂"].time == 0 and aura_env.spell["死亡印记"].cd <= 10 and aura_env.resource.combo_points > 0 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "割裂"  
        --切割：切割buff = 0，连击点 > 0，10码总人数 <= 1         
        elseif aura_env.aura["切割"].time == 0 and aura_env.resource.combo_points > 0 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "切割"                 
    ...
    elseif aura_env.loop_id == 21 then
        ...
        if (aura_env.aura["强化锁喉-潜行"].time > 0 or aura_env.aura["诡诈"].time > 0) and aura_env.aura["死亡猎手标记"].time == 0 and aura_env.resource.combo_points < 5 then
            next_spell.name = "伏击"        
        --割裂：割裂buff = 0，死亡印记cd <= 10，连击点 > 0，10码总人数 <= 1         
        elseif aura_env.aura["割裂"].time == 0 and aura_env.spell["死亡印记"].cd <= 10 and aura_env.resource.combo_points > 0 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "割裂"      
        --切割：切割buff = 0，连击点 > 0，10码总人数 <= 1         
        elseif aura_env.aura["切割"].time == 0 and aura_env.resource.combo_points > 0 and aura_env.enemy.num_in_range_10 <= 1 then
            next_spell.name = "切割"              
    ...
    elseif aura_env.loop_id == 31 then
        ...
        if aura_env.spell["申斥回响"].cd > aura_env.burst.cd + aura_env.burst.time then
            aura_env.status.combo_points_shen_chi_hui_xiang = false
        end                
        --割裂：0 < 割裂buff < 1，连击点 > 0         
        if aura_env.aura["割裂"].time > 0 and aura_env.aura["割裂"].time < 1 and aura_env.resource.combo_points > 0 then
    ...
    elseif aura_env.loop_id == 45 then
        --申斥回响豆
        if aura_env.resource.combo_points < 5 and aura_env.aura["至黑之夜"].time > 0 then
        --申斥回响豆禁用：申斥回响cd > 爆发cd + 爆发时间
    ...
    elseif aura_env.loop_id == 42 then
        ...
        if aura_env.spell["申斥回响"].cd > aura_env.burst.cd + aura_env.burst.time + 5 then
            aura_env.status.combo_points_shen_chi_hui_xiang = false
        end        
    ...
    elseif aura_env.loop_id == 53 then
        if aura_env.aura["君王之灾"].time > 0 and aura_env.aura["君王之灾"].time > aura_env.aura["死亡猎手标记"].stack * 4 + 5 then
            aura_env.status.combo_points_shen_chi_hui_xiang = false
        end
    ...
    elseif aura_env.loop_id == 52 then
        if aura_env.aura["死亡印记"].time > 0 or aura_env.aura["死亡猎手标记"].stack <= 1 then
            aura_env.status.combo_points_shen_chi_hui_xiang = false
        end        
        --死亡印记：死亡印记cd = 0
...
aura_env.CalculateSpellBySpellType = function()
    ...
    elseif aura_env.next_spell.type == 1 then
        --刀扇：10码目标 > 2
        if aura_env.enemy.num_in_range_8 > 2 or (aura_env.enemy.num_in_range_8 == 2 and (aura_env.aura["夺命药膏"].effect_unit_num == 0 or (aura_env.aura["夺命药膏"].effect_unit_num == 1 and aura_env.aura["夺命药膏"].time > 0)) and (not aura_env.talent["凶残毒素"])) then
...
aura_env.UpdateResouceByCastSpell = function(_index)
    ...
    if aura_env.next_spell.name == "刀扇" then
        if aura_env.aura["见者尽灭"].time > 0 then
            aura_env.resource.combo_points = math.min(aura_env.resource.combo_points + 2 + math.floor(aura_env.enemy.num_in_range_8 * 0.2), aura_env.resource.combo_points_max)
            aura_env.aura["见者尽灭"].time = 0
        else
            aura_env.resource.combo_points = math.min(aura_env.resource.combo_points + 1 + math.floor(aura_env.enemy.num_in_range_8 * 0.2), aura_env.resource.combo_points_max)
        end
        aura_env.resource.energy = aura_env.resource.energy - 35
        aura_env.aura["增效药膏"].time = 12
        aura_env.aura["夺命药膏"].time = 12
        aura_env.aura["夺命药膏"].effect_unit_num = aura_env.enemy.num_in_range_8
    ...
    elseif aura_env.next_spell.name == "锁喉" then
        ...
        if aura_env.aura["连环屠戮-潜行"].time > 0 or aura_env.aura["连环屠戮"].time > 0 then
            aura_env.aura["锁喉"].effect_unit_num = math.min(aura_env.aura["锁喉"].effect_unit_num + 3, aura_env.enemy.num_in_range_8)
        end
    ...
    elseif aura_env.next_spell.name == "割裂" then
        ...
        if aura_env.talent["死亡猎手标记"] then
            aura_env.aura["割裂"].time = aura_env.aura["割裂"].time + 3
        end
        aura_env.aura["潜行"].time = 0
        ...
        if aura_env.talent["锯齿骨刺"] and aura_env.stack_aura["锯齿骨刺-自身"].stack > 0 then
            if aura_env.aura["连环屠戮-潜行"].time > 0 or aura_env.aura["连环屠戮"].time > 0 then
                local hit_num = math.min(aura_env.enemy.num_in_range_8, aura_env.stack_aura["锯齿骨刺-自身"].stack)
                ...
                aura_env.aura["割裂"].effect_unit_num = math.min(aura_env.aura["割裂"].effect_unit_num + 3, aura_env.enemy.num_in_range_8)
            else
    ...
    elseif aura_env.next_spell.name == "猩红风暴" then
        ...
        aura_env.resource.combo_points = 0
        aura_env.resource.energy = aura_env.resource.energy - 45
        aura_env.aura["锯齿骨刺-敌方"].effect_unit_num = aura_env.enemy.num_in_range_8
