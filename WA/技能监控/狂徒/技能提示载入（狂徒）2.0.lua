aura_env.CFG = {
    ...
    --需要更新的光环名字和id
    AuraList = {
        {Name = "锋化之刃", ID = 441786, IfSelfAura = true, IfHelpfulAura = true, IfCheckEffectUnitNum = false, TimeMax = 9999},
        ...
        {Name = "敏锐", ID = 193538, IfSelfAura = true, IfHelpfulAura = true, IfCheckEffectUnitNum = false, TimeMax = 9999},
...
aura_env.UpdateStatus = function() 
    ...
    aura_env.status.dice_min_time = 9999
    for i = 1, #aura_env.CFG.DiceNameList do
        local dice = aura_env.CFG.DiceNameList[i]
        ...
        if aura_env.status.dice_min_time > aura_env.aura[dice].time and aura_env.aura[dice].time > 0 then
            aura_env.status.dice_min_time = aura_env.aura[dice].time
        end      
    end
    if aura_env.status.dice_min_time == 9999 then aura_env.status.dice_min_time = 0 end

    --骰子权重计算
    local dice_weight = 0
    aura_env.status.unlock_dice_weight = 0
    aura_env.status.dice_weight = 0
    aura_env.status.dice_num = 0
    --强势连击
    if aura_env.aura["强势连击"].time > 0 then
        aura_env.status.dice_num = aura_env.status.dice_num + 1
        dice_weight = 1.3
    ...
    aura_env.status.dice_reroll = aura_env.status.dice_num < 5
    --骰子优先级计算
    ...
    -- --权重+0.6：超荷充能
    -- if aura_env.talent["超荷充能"] then dice_weight_need = dice_weight_need + 0.6 end
    -- if aura_env.status.dice_max_time >= 30 then     
    --     aura_env.status.dice_reroll = false
    -- elseif aura_env.spell["时运继延"].cd == 0 and (aura_env.status.dice_weight > 3 or aura_env.status.dice_num >= 4) then    
    --     aura_env.status.dice_reroll = false
...
aura_env.UpdateBurstInfo = function()
    ...
    if aura_env.aura["强势连击"].time > 5 then 
        aura_env.burst.factor = aura_env.burst.factor + 0.8
    else
    ...
    --消失cd > 0
    if aura_env.spell["消失"].cd > 0 then
...
aura_env.SpecialSpellDisplay = function()
    ...
    if aura_env.talent["时运继延"] then
        ...
        --显示：时运继延cd = 0，(骰子权重 > 3 | 骰子数量 >= 4)
        --if aura_env.spell["时运继延"].cd == 0 and (aura_env.status.dice_weight > 3 or aura_env.status.dice_num >= 4) then
        if aura_env.spell["时运继延"].cd == 0 and aura_env.status.dice_num >= 5 and aura_env.status.dice_min_time > 25 then
            region:Show()
        --不显示
        else   
            region:Hide()
...
aura_env.CalculateSpellTypeByLoopID = function(_if_cast_now)
    ...
    if aura_env.loop_id == 10 then       
        ...
        --剑刃乱舞：矫捷灵动天赋，剑刃乱舞cd = 0，剑刃乱舞buff = 0，（冲动cd = 0 | 冲动buff > 潜行cd），连击点 >= 5
        elseif aura_env.talent["矫捷灵动"] and aura_env.spell["剑刃乱舞"].cd == 0 and aura_env.aura["剑刃乱舞"].time == 0 and (aura_env.spell["冲动"].cd == 0 or aura_env.aura["冲动"].time > aura_env.spell["潜行"].cd) and aura_env.resource.combo_points >= 5 then
            next_spell.name = "剑刃乱舞"
        --潜行：潜行cd = 0，非隐身
        ...
        elseif aura_env.aura["潜行"].time == 0 then            
            next_spell.name = "潜行"
        --切割：0 < 敏锐buff <= 1，连击点 > 0
        elseif aura_env.aura["敏锐"].time > 0 and aura_env.aura["敏锐"].time <= 1 and aura_env.resource.combo_points > 0 then
            next_spell.name = "切割"                      
        --命运骨骰：超荷充能天赋，命运骨骰cd = 0，0 < 敏锐buff <= 3
        elseif aura_env.talent["超荷充能"] and aura_env.spell["命运骨骰"].cd == 0 and aura_env.aura["敏锐"].time > 0 and aura_env.aura["敏锐"].time <= 3 then
            next_spell.name = "命运骨骰"                 
        --循环切换
        else
            aura_env.loop_id = 40
    ...
    elseif aura_env.loop_id == 20 then
        ...
        elseif aura_env.spell["冲动"].cd == 0 and aura_env.aura["冲动"].time < 0.8 and aura_env.resource.combo_points <= 2 then
            next_spell.name = "冲动"               
        --技能类型1：锋化之刃层数 = 4 | （影舞步天赋，影舞步cd = 0），连击点 < 7
        elseif (aura_env.aura["锋化之刃"].stack == 4 or (aura_env.talent["影舞步"] and aura_env.spell["影舞步"].cd == 0)) and aura_env.resource.combo_points < 7 then
            next_spell.type = 1          
        --斩击：锋化之刃层数 = 4，连击点 == 7
        elseif aura_env.aura["锋化之刃"].stack == 4 and aura_env.resource.combo_points == 7 then
            next_spell.name = "斩击"            
        --影舞步：影舞步天赋，影舞步cd = 0，连击点 == 7
        elseif aura_env.talent["影舞步"] and aura_env.spell["影舞步"].cd == 0 and aura_env.resource.combo_points == 7 then
            next_spell.name = "影舞步"            
        --技能类型2：连击点 >= 5
