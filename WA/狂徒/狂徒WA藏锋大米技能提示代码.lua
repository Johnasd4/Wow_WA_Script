function(allstates,event,...)
    ...
    if event == "FRAME_UPDATE" then
        ...
        if loopID == 2 then
            ...
            --连击点≤6(强势连击buff>0则为5)，消失cd>[鬼魅攻击冷却]，暗影之舞cd>([鬼魅攻击冷却]-连击点(精准定位buff>0则*2))*0.33(精准定位buff>0则*0.2)*冲动buff系数(0.86(常数))
            if aura_env.spell.GMGJ.cd == 0 and aura_env.comboPoints <= 6 and aura_env.spell.XS.cd > aura_env.cooldownGMGJ and aura_env.spell.AYZW.cd > (aura_env.cooldownGMGJ - aura_env.comboPoints * factorJZDW_1) * factorJZDW_2 * factorCD then
                showGMGJ = true
                --连击点+1（强势连击buff>0则+2）
                if aura_env.aura.QSLJ.time > 0 then
        ...
        if loopID == 4 then
            ...
            or (aura_env.spell.CD.cd == 0 and aura_env.comboPoints <= 2 and aura_env.spell.ZZMX.cd > 0 and aura_env.aura.CD.time <= aura_env.loop4supplyTimeCD) then
                --冲动，连击点视为7，冲动buff=20，灌铅骰子buff=45，骰子重新判定
                showCD = true
                aura_env.comboPoints = aura_env.comboPointsMax
                aura_env.burstComboPoints = aura_env.comboPointsMax
            ...
            if (aura_env.spell.JRLW.cd == 0 and aura_env.aura.JRLW.time == 0 and aura_env.enemyCount > 1)
        ...
        if spellTypeID == 1 then
            ...
            if aura_env.burstTime > 0 then
                ...
                or (aura_env.aura.KCZJ.stack > 0 and aura_env.comboPoints <= 3 and aura_env.aura.QSLJ.time == 0)
                or (aura_env.aura.KCZJ.stack > 0 and aura_env.comboPoints <= 2 and aura_env.aura.QSLJ.time > 0) then
        ...
        -- 设置额外图标可见
        aura_env.updateExtraIconVisible("技能提示：大米藏锋—冲动", showCD)          
        aura_env.updateExtraIconVisible("技能提示：大米藏锋—消失", showXS)           
        aura_env.updateExtraIconVisible("技能提示：大米藏锋—暗影之舞", showAYZW)    
        aura_env.updateExtraIconVisible("技能提示：大米藏锋—鬼魅攻击", showGMGJ)    
